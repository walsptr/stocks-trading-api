...
from stocks_trading.domain.models import DailyRules, ScoreSymbolStatus
...
class FakeScoreRepository:
    ...
    def latest_score_date(self, symbol, version, checksum):
        return date(2026, 7, 10)

    def load_rule_results(self, symbol, *, formula_version, config_checksum, start_date=None, end_date=None):
...
class FakeRunRepository:
    ...
    def create_score_run(self, request):
        self.request = request
        return self.run_id

    def record_score_symbol_result(self, run_id, result):
        self.results.append(result)

    def finish_score_run(self, run_id):
        self.finished = run_id

    def abandon_score_run(self, run_id):
        self.abandoned = run_id


def daily_rules(symbol):
    return DailyRules(
        symbol=symbol, trading_date=date(2026, 7, 16),
...
def service(repository):
    return ScoringService(
        ...
        configuration=load_scoring_configuration(Path("config/scoring/technical-v1.yaml")),
    )
...
async def test_update_uses_seven_day_overlap() -> None:
    ...
    result = await service(repository).update(symbols=["BBCA.JK"], as_of=date(2026, 7, 16))
    assert result.failed_count == 0
    assert repository.load_calls[0][3] == date(2026, 7, 3)
    assert repository.written[0].score == 100


@pytest.mark.asyncio
async def test_failure_isolated() -> None:
    result = await service(FakeScoreRepository(fail_symbol="TLKM.JK")).rebuild(
        symbols=["BBCA.JK", "TLKM.JK"]
    )
    assert result.failed_count == 1
    assert {item.status for item in result.symbols} == {
        ScoreSymbolStatus.SUCCESS, ScoreSymbolStatus.FAILED
    }


@pytest.mark.asyncio
async def test_no_rules_records_no_data() -> None:
    result = await service(FakeScoreRepository(no_data=True)).rebuild(symbols=["BBCA.JK"])
    assert result.symbols[0].status == ScoreSymbolStatus.NO_DATA
